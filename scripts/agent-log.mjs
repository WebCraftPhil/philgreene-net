...
if (!intentRaw) {
  console.error('Usage: agent-log <intent> [inputs-json]');
  process.exit(1);
}
...
const id =
  utc
    ...
    .slice(0, 12) +
  '-' +
  intent.replace(/\s+/g, '-').toLowerCase().slice(0, 28);
const fp = crypto
  .createHash('sha256')
  .update(JSON.stringify({ intent, inputs }), 'utf8')
  .digest('hex');
...
const dup = arr.find(
  e => e.fingerprint === `sha256:${fp}` && e.status === 'done'
);
if (dup) {
  arr.push({
    id,
    ...
    evidence: { prior: dup.id },
    notes: ['Auto-skip duplicate'],
  });
...
const related = fs.existsSync('.git')
  ? execSync('git diff --cached --name-only || true')
      .toString()
      .trim()
      .split('\n')
      .filter(Boolean)
