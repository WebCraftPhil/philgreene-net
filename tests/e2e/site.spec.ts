...
test('Website Checkup scans, unlocks the report, and prefills the audit', async ({ page }) => {
  ...
  await expect(page.locator('link[rel="canonical"]')).toHaveAttribute('href', 'https://philgreene.net/website-checkup')
  await expect(page.locator('meta[property="og:image"]')).toHaveAttribute('content', 'https://philgreene.net/website-checkup-og.svg')
  await expect(page.locator('meta[name="twitter:image"]')).toHaveAttribute('content', 'https://philgreene.net/website-checkup-og.svg')
  await page.getByLabel('Website address').fill('roofer.example')
  const scanButton = page.getByRole('button', { name: 'Scan My Website' })
  await expect(scanButton).toBeEnabled()
  await scanButton.click()
