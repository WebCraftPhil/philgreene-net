  ...
  return (
    <section id="packages" className="pilot-section section" aria-labelledby="packages-heading">
      <div className="site-container">
        ...
        <div className="package-grid">{packages.map((item) => <article key={item.id} className={item.featured ? 'package-featured' : ''}><span>{item.eyebrow}</span><h3>{item.name}</h3><p>{item.description}</p><ul>{item.items.map((feature) => <li key={feature}><Check aria-hidden="true" />{feature}</li>)}</ul><button className={item.featured ? 'button button-primary' : 'button button-outline'} type="button" aria-label={`Ask about the ${item.name} package`} onClick={() => onSelectPackage(item.id)}>Ask about this package <ArrowRight aria-hidden="true" /></button></article>)}</div>
